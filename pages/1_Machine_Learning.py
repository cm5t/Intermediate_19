...
st.set_page_config(page_title="Our Machine Learning Model")
st.sidebar.header("Machine Learning Model")
st.sidebar.write("This page showcases the machine learning model used to predict saving urgency based on family income and expenditure data.")
...
dectree = RandomForestClassifier(n_estimators=500, random_state=69)
dectreefitted = dectree.fit(X_train, y_train)

y_pred = dectreefitted.predict(X_test)
print("Accuracy:", accuracy_score(y_test, y_pred))

importances = dectreefitted.feature_importances_
...
st.title("We used a decision tree model.")
st.header("Accuracy: " + str(accuracy_score(y_test, y_pred) * 100) + "%")
onetree = dectree.estimators_[0]
tree.plot_tree(onetree, feature_names=X.columns.tolist())
st.pyplot(plt)

n_paths = sum(onetree.tree_.children_left == -1)
print(f"Number of decision paths (leaf nodes): {n_paths}")
